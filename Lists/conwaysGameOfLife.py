...
def image():
    ...
    for y in range(len(field)):         # E.g. field[3]
        for x in range(len(field[y])):  # E.g. field[3][2]
            ...
            # 1. rule
            if field[y][x]: # works only if zeroes and ones
                if liveNeighbours == 2 or liveNeighbours == 3:
                    endField[y][x] = field[y][x]
                else:
                    endField[y][x] = 0
...
for i in range(int(input('Enter a number of iterations: '))):
    image()
    field = copy.deepcopy(endField)
